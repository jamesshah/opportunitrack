...
import { getEmails } from "../services/nylas.service.js";
import { Webhooks } from "nylas/lib/types/resources/webhooks.js";
...
router.post("/webhook", async (req, res) => {
	...
	try {
		...
		if (webhookMessage.type == "message.created") {
			...
			const message: Message = webhookMessage.data.object;
			try {
				const response = await RedisClient.lPush(
					"process-email-queue",
					JSON.stringify({
